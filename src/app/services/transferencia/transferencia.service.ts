...
export class TransferenciaService {
  ...
  token:string;
  loginURL:string = environment.URL_API+"transferencia";
  
  
  constructor(private httpClient: HttpClient) { }

 getListado():Observable<RequestResponse>{
     return  this.httpClient.get<RequestResponse>(environment.URL_API+"transferencias");
  }

registro(destinatario: Destinatario, monto:number): Observable<RequestResponse> {
    const data = {
      persona: {
